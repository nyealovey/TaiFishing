...
# 配置文件
conf_file="/mysql_backup_8/conf/mysql_increment_hot_backup.conf"
# mysql 用户
...
# 全备是在当天的时辰
#full_backup_hour=`sed '/^full_backup_hour=/!d;s/.*=//' $conf_file`
# mysql 全备前缀标识
...
# 创建相关目录
log_dir='/mysql_backup_8/log'
var_dir='/mysql_backup_8/var'
#mkdir -p $backup_dir
mkdir -p $log_dir
mkdir -p $var_dir
...
# full_backup_hour=`sed '/^full_backup_hour=/!d;s/.*=//' $conf_file`
  backup_type=0
  if [ "$full_backup_week_day" -eq `date +%u` ]; then  #周全备
#	if [ "$full_backup_hour" -eq `date +%H` ]; then		#日全备
		backup_type=0
#  	else
#		backup_type=1
#  	fi
...
#  if [ ! -n "$full_backup_hour" ]; then echo 'fail: configure file full_backup_hour not set'; exit 2; fi
