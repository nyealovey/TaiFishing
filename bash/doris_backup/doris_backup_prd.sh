#!/bin/bash
# =====================================================
# Doris 2.1 改进版备份脚本（增强 Crontab 兼容性）
# 变更：自动检测环境、支持绝对路径、增强错误诊断
# =====================================================
#
# 环境变量说明及示例请查看 `bash/doris_backup/env.example`
#
# 运行示例：
#   bash bash/doris_backup/doris_backup_prd.sh
#   DORIS_USER=admin bash bash/doris_backup/doris_backup_prd.sh
#
# Crontab 示例：
...
# 1. 自动定位脚本所在目录
DORIS_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly DORIS_SCRIPT_DIR

# 2. 自动定位仓库根目录
DORIS_REPO_ROOT="$(cd "$DORIS_SCRIPT_DIR/../.." && pwd)"
readonly DORIS_REPO_ROOT

# 3. 提前加载 .env 配置（让用户配置优先生效）
DORIS_ENV_FILE_PATH="${DORIS_ENV_FILE:-$DORIS_SCRIPT_DIR/.env}"
if [[ -f "$DORIS_ENV_FILE_PATH" ]]; then
  set -a
  # shellcheck disable=SC1090
  source "$DORIS_ENV_FILE_PATH"
  set +a
  echo "[INFO] 已加载环境配置: $DORIS_ENV_FILE_PATH"
else
  echo "[WARN] 未找到 .env 文件: $DORIS_ENV_FILE_PATH，使用默认配置"
fi
readonly DORIS_ENV_FILE=${DORIS_ENV_FILE:-$DORIS_ENV_FILE_PATH}

# 4. 自动查找 MySQL 客户端（优先使用环境变量，否则自动搜索）
if [[ -z "${DORIS_MYSQL_BIN:-}" ]]; then
  for MYSQL_PATH in \
    /usr/bin/mysql \
    ...
    if [[ -x "$MYSQL_PATH" ]]; then
      DORIS_MYSQL_BIN="$MYSQL_PATH"
      break
    fi
  done
  
  if [[ -z "${DORIS_MYSQL_BIN:-}" ]]; then
    echo "[FATAL] 无法找到 MySQL 客户端，请设置 DORIS_MYSQL_BIN 环境变量" >&2
    exit 1
  fi
fi
readonly DORIS_MYSQL_BIN
...
readonly DORIS_LOG_DIR=${DORIS_LOG_DIR:-"/opt/backup/backup_logs"}
...
# -----------------------------------------------------------------------------
run_mysql() {
  # 使用 MYSQL_PWD 环境变量传递密码，避免命令行泄露和交互式提示
  MYSQL_PWD="$DORIS_PASSWORD" "$DORIS_MYSQL_BIN" \
    -h"$DORIS_FE_HOST" \
...
log() {
  ...
  echo "$msg"
  echo "$msg" >> "$DORIS_LOG_FILE"
}

# -----------------------------------------------------------------------------
# 函数: check_prerequisites
# 说明: 检查运行环境（MySQL 客户端、网络连接、日志目录）
# 返回: 检查通过返回 0，否则 1
# -----------------------------------------------------------------------------
check_prerequisites() {
  log "========== 环境检查 =========="
  log "脚本目录: $DORIS_SCRIPT_DIR"
  log "仓库根目录: $DORIS_REPO_ROOT"
  log "MySQL 客户端: $DORIS_MYSQL_BIN"
  log "环境配置文件: $DORIS_ENV_FILE"
  log "日志目录: $DORIS_LOG_DIR"
  
  # 检查 MySQL 客户端可执行
  if [[ ! -x "$DORIS_MYSQL_BIN" ]]; then
    log "【错误】MySQL 客户端不可执行: $DORIS_MYSQL_BIN"
    return 1
  fi
  
  # 检查日志目录
  if ! mkdir -p "$DORIS_LOG_DIR" 2>/dev/null; then
    log "【错误】无法创建日志目录: $DORIS_LOG_DIR"
    return 1
  fi
  
  # 测试数据库连接
  log "测试 Doris FE 连接: $DORIS_FE_HOST:$DORIS_FE_QUERY_PORT"
  if ! run_mysql -e "SELECT 1;" >/dev/null 2>&1; then
    log "【错误】无法连接到 Doris FE，请检查："
    log "  - 主机地址: $DORIS_FE_HOST"
    log "  - 端口: $DORIS_FE_QUERY_PORT"
    log "  - 用户名: $DORIS_USER"
    log "  - 密码: [已隐藏]"
    log "  - 网络连通性"
    return 1
...
if ! check_prerequisites; then
  log "【严重错误】环境检查失败，退出备份"
  exit 1
fi

# 获取数据库列表
log "正在查询待备份数据库列表..."
if ! DATABASES=$(run_mysql -N -s -e "
  SELECT SCHEMA_NAME 
  FROM information_schema.SCHEMATA 
  WHERE SCHEMA_NAME NOT IN ('information_schema','__internal_schema','mysql','ctl','sys') 
